...
class ParametersModel(QAbstractTableModel):
    ...
    def formula_to_locale(self, formula):
        locale = QLocale()
        if locale.decimalPoint() == ",":
            return formula.replace(",", ";").replace(".", ",")
        return formula

    def formula_from_locale(self, formula):
        locale = QLocale()
        if locale.decimalPoint() == ",":
            return formula.replace(",", ".").replace(";", ",")
        return formula
        ...
        if int_role == Qt.DisplayRole:
            ...
            if col == 0:
                ...
                if param_item is Parameters:
                    ...
                    param = param_item
                data = self.formula_to_locale(param.formula)
            elif col == 2:
                if param_item is Parameters:
                    param = param_item.get_parameter(col - 2)
                else:
            ...
            if col == 0:
                ...
                if param_item is Parameters:
                    ...
                    param = param_item
                if param.formula != "":
                    data = data = self.formula_to_locale(param.formula)
                else:
            ...
            if parsed[1]:
                ...
                try:
                    if value == "":
                        ...
                        param.value = self.formula_from_locale(value)
